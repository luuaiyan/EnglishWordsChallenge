...
1. 数据概览增加可选项：近7天，15天，30天
2. 底部增加相应天数的正确率趋势图表

### v1.05 (2026.03.27)
每次测试结束后，显示该次测试结果，并且增加图表显示近7次测试准确率曲线

### v1.04 (2026.03.26)
光荣榜的判断依据为每月1号至月底测试准确率，每月1号重置
...
毕竟项目创建的初衷，是给班级的小朋友日常学习相关词汇的积累使用，最大的原因也是全国每个地区学习的内容都不一致，无法将每个年级每个学期学习的单词一次性做到位。
...
本项目由本人开发并维护。项目代码采用 **MIT License** 开源，任何人可以自由地进行分发、修改和个人使用。

## 免责条款
1. **数据安全**：本项目仅作为英语学习辅助工具。开发者不承担因服务器故障、数据丢失或软件 Bug 导致的任何直接或间接损失（如学习记录丢失等）。
2. **内容准确性**：本项目的自动词库生成脚本依赖第三方词典数据。开发者不保证所有单词音标、释义的 100% 准确性，建议在使用前由家长或老师进行人工核对。
3. **教育用途**：本项目仅限用于非营利性的教育和学习用途。严禁将本项目用于任何形式的非法牟利活动。
4. **第三方资源**：项目中使用的 TTS 发音引擎为浏览器内置服务，其稳定性受限于用户设备及系统环境。
